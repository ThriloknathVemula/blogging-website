...
export const Appbar = ()=>{
    ...
    return(
        <div className="border-b flex justify-between p-4 md:px-10 md:py-4">
            <Link to={token ? '/blogs' : '/signin'} className="flex text-xl md:text-3xl flex-col justify-center cursor-pointer font-bold">
                Medium
            </Link>
            <div className="flex items-center gap-4">
                {token && <Link to={'/publish'} className="cursor-pointer px-2 py-1 rounded-md flex items-center lg:gap-1 bg-green-500 text-slate-100">
                ...
                <div className="md:hidden">
                    {token ? <IoIosLogOut onClick={logoutFn} className="h-6 w-6 cursor-pointer"/> : <IoIosLogIn onClick={logoutFn} className="h-6 w-6 cursor-pointer"/>}
                </div>
            </div>
        </div>
    )
